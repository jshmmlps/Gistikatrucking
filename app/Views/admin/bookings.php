                ...
                <div class="modal-content">
                  <form action="<?= base_url('admin/update-booking-status') ?>" method="post" id="updateForm<?= $booking['booking_id'] ?>">
                    ...
                    <div class="modal-body">
                      ...
                      <!-- Dropdown to select a new Driver -->
                      <!-- <div class="mb-3">
                        <label for="driver<?= $booking['booking_id'] ?>" class="form-label">Select Driver:</label>
                        <select name="driver" id="driver<?= $booking['booking_id'] ?>" class="form-select updateDriver">
                          <option value="">-- Select Driver --</option>
                          <?php if(isset($availableDrivers) && is_array($availableDrivers)): ?>
                            <?php foreach($availableDrivers as $driver): ?>
                              <option value="<?= $driver['driver_id'] ?>" 
                                data-truck="<?= $driver['truck_assigned'] ?>" 
                                data-truck-model="<?= $driver['truck_model'] ?? '' ?>">
                                <?= $driver['full_name'] ?>
                              </option>
                            <?php endforeach; ?>
                          <?php endif; ?>
                      ...
                      <!-- Dropdown to select a new Conductor -->
                      <!-- <div class="mb-3">
                        <label for="conductor<?= $booking['booking_id'] ?>" class="form-label">Select Conductor:</label>
                        <select name="conductor" id="conductor<?= $booking['booking_id'] ?>" class="form-select updateConductor">
                          <option value="">-- Select Conductor --</option>
                          <?php if(isset($availableConductors) && is_array($availableConductors)): ?>
                            <?php foreach($availableConductors as $conductor): ?>
                              <option value="<?= $conductor['driver_id'] ?>">
                                <?= $conductor['full_name'] ?>
                              </option>
                            <?php endforeach; ?>
                          <?php endif; ?>
...
<!-- JavaScript to update truck details when driver or conductor selection changes
<script>
document.addEventListener('DOMContentLoaded', function(){
  // For each booking modal, listen for changes on the driver dropdown
  document.querySelectorAll('.updateDriver').forEach(function(driverSelect){
    driverSelect.addEventListener('change', function(){
      var bookingId = this.id.replace('driver','');
      var selectedOption = this.options[this.selectedIndex];
      var truckId = selectedOption.getAttribute('data-truck');
      var truckModel = selectedOption.getAttribute('data-truck-model');
      
      // Update the hidden truck_id field if a driver is selected
      if(truckId){
        document.getElementById('truck_id' + bookingId).value = truckId;
        // Also update the displayed truck model if available
        if(truckModel){
          document.getElementById('truckModel' + bookingId).innerText = truckModel;
        }
      }
    });
  });
  
  // Optionally, you can add a similar event listener for the conductor dropdown if needed.
});
</script> -->
